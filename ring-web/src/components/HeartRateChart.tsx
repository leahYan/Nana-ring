...
export default function HeartRateChart({ userId }: Props) {
  const [range, setRange] = useState<Range>('24h')
  const since = Date.now() - RANGE_MS[range]

  const { data, isLoading } = useQuery({
    queryKey: ['hr-chart', userId, range],
    queryFn: async () => {
      ...
      return (data ?? []) as { device_timestamp: number; bpm: number }[]
    },
  })
  ...
  return (
    <section className="bg-white rounded-2xl border border-stone-200 p-6 shadow-sm">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="text-xl font-semibold text-stone-800">Heart Rate</h2>
          <p className="text-sm text-stone-500 mt-0.5">beats per minute (bpm)</p>
        </div>
      ...
      {!isLoading && (!data || data.length === 0) && (
        <p className="text-base text-stone-400 py-14 text-center italic">
          No heart rate readings in the last {range}. Make sure the ring app is syncing.
        </p>
      )}
